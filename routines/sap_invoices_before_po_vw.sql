  ...
  bkpf_bseg AS (
    ...
    WHERE
      ...
      AND PARSE_DATE('%Y%m%d', CAST(H_Document_Date AS STRING))
        between DATE_SUB(CURRENT_DATE(), INTERVAL 24 MONTH) and CURRENT_DATE()
      AND bseg.H_Document_Type IN ('PX', 'PY', 'VS', 'Y0', 'YM')
  ),
  doc AS (
    SELECT DISTINCT
    ...
    FROM
      gcp-wow-ent-im-tbl-prod.adp_dm_purchasing_view.document_schedule_lines_v
        sl
    WHERE
      PurchasingDocumentDate between DATE_SUB(CURRENT_DATE(), INTERVAL 24 MONTH) and CURRENT_DATE()
      AND PurchasingOrganization = '1000'
  ),
  unagg_po_invoice AS (
    SELECT bseg.*,sl.* ,v.VendorDescription,Supplier_Custom_Category_L2
    FROM bkpf_bseg bseg
    INNER JOIN
      doc sl
