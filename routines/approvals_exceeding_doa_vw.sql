CREATE OR REPLACE VIEW fraud.approvals_exceeding_doa_vw
AS
WITH
  approvals AS (
    SELECT DISTINCT
  ...
  -- first_approval as (
  -- select distinct Approvable_ID,  Approver, row_number() over (partition by Approvable_ID order by Action_Date asc) as Z from  approvals)
  first_approval AS (
    SELECT
      Approvable_ID,
  ...
  f AS (
    ...
    FROM Y
    WHERE
      PO_Number <> 'Unclassified' AND Annual_Limit_Check_Last_Approver = 'Yes'
  )
SELECT DISTINCT
  Supplier_Custom_Category_L2 AS vendor_area,
  'Payments' AS routine_category,
  'Ariba - Approvals made by employees without appropriate DOA or exceeding their DOA'
