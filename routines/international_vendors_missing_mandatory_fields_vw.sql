CREATE OR REPLACE view fraud.international_vendors_missing_mandatory_fields_vw
AS
-- Local Vendor
WITH
  vendors AS (
  ...
  basevendor AS (
    SELECT DISTINCT
      ...
      z1.ContactEmail,
      z3.ContactPersonFirstName AS finance_ContactPersonFirstName,
      z3.ContactPersonLastName AS finance_ContactPersonLasttName,
      z3.ContactTelephone AS finance_ContactTelephone,
      z3.ContactFax AS finance_ContactFax,
      z3.ContactEmail AS finance_ContactEmail,
    ...
    LEFT JOIN
      (
        ...
        FROM
          gcp-wow-ent-im-tbl-prod.adp_dm_masterdata_view.dim_vendor_purchorg_function_v
        WHERE ContactPerson_FunctionID = 'Z3'
      ) z3
      ON a.vendor = z3.vendor
    LEFT JOIN
    ...
    WHERE
      ...
      AND (
        ...
        OR (z3.ContactPersonFirstName IS NULL OR z3.ContactPersonFirstName = '')
        OR (z3.ContactPersonLastName IS NULL OR z3.ContactPersonLastName = '')
        OR (z3.ContactTelephone IS NULL OR z3.ContactTelephone = '')
        OR (z3.ContactFax IS NULL OR z3.ContactFax = '')
        OR (z3.ContactEmail IS NULL OR z3.ContactEmail = '')
        OR (IncoTerms2 IS NULL OR IncoTerms2 = '')
        OR (SchemaGroupVendor IS NULL OR SchemaGroupVendor = '')
...
SELECT
  ...
  'Master data completeness for International vendors' AS routine_description,
  'International Vendors Count' AS metric_description,
